...
import { Kiwiland, InputLink, Route, NOROUTE } from "./kiwiland4";
const testsCases: number[] = [45];
...
checkAndPrint(kiwiland.routeDistance("AED"), NOROUTE);
...
export function printroutes(routes: string[]) {
  routes.forEach( r => {
    console.log(r);
  }) 
...
export function checkRoutesAndPrint(response: Route[], answer: string[]) {
  ...
  let allRoutesDiscovered = answer.every( s => response.some( r => r==s ) )
