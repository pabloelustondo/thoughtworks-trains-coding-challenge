...
export type Route = string; // a route is just a sequence of towns incuding loops such as CEBCEBCEBC
...
export class Kiwiland {
  ...
  routeDistance(route: Route): Distance {
    ...
    let distance, totalDistance : Distance = 0;
    for( let i=1; i< route.length;i++){
      distance = this.linkDistance(route[i - 1], route[i]);
      if (distance != NOROUTE){
        totalDistance += distance;
      } else {
        totalDistance = NOROUTE;
      }
    }
